...
cam = cv2.VideoCapture("http://192.168.1.44:8080/video")
cam.set(3,640)
cam.set(4,480)
count = 0
...
#     for filename in os.listdir("Real-Time-Face-Recognition/mo"):
#         img = cv2.imread(os.path.join("Real-Time-Face-Recognition/mo", filename))
#         gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
#         faces = faceCascade.detectMultiScale(gray, 1.3, 5)
#         for (x, y, w, h) in faces:
#             cv2.rectangle(img, (x, y), (x + w, y + h), (255, 0, 0), 2)
